...
@php
    $tags = "";
    $lastElement = $event->tags[count($event->tags)-1];
...
<form method="POST" action="{{ route('events.update', $event) }}">
    ...
    <div class="details">
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" autocomplete="off" value="{{ $event->name }}">
        </div>
        <div class="form-group">
            <label for="start-time">When does your event start</label>
            <input type="datetime-local" id="start-time" name="start-time" value="{{ \Carbon\Carbon::parse($event->start_date)->format("yy-m-d\Th:i") }}">
        </div>

        <div class="form-group">
            <label for="tags">Please enter some tags to help us market your event (Comma seperated)</label>

            <input type="textbox" id="tags" name="tags" autocomplete="off" value="{{ $tags }}">
        </div>
    </div>
    <input type="hidden" id="map_latitude" name="latitude" value={{ $event->latitude }}>
    <input type="hidden" id="map_longitude" name="longitude" value={{ $event->longitude }}>
    ...
    <input type="submit" value="Update your event">
</form>
...
@section('scripts')
    ...
    <script>
        var map;
        function initMap() {
            var myLatlng = new google.maps.LatLng({{ $event->latitude }},
{{ $event->longitude }});
